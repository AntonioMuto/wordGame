...
class CrosswordPage extends StatelessWidget {
      ...
      child: Scaffold(
        ...
        body: Column(
          ...
          children: [
            ...
            // Banner Ads
            BlocBuilder<CrosswordBloc, CrosswordState>(builder: (context, crosswordState) {
              if (crosswordState is CrosswordLoaded) {
                return BlocBuilder<AdsBloc, AdsState>(
                  builder: (context, adsState) {
                    if (adsState is AdsLoading) {
                      return Center(child: Container(margin: EdgeInsets.only(bottom: 15), child: CircularProgressIndicator()));
                ...
                return Padding(
                  ...
                  child: ElevatedButton(
                    onPressed: () {
                      ...
                      context.read<AdsBloc>().stream.listen((state) {
                        if (state is InterstitialAdLoaded) {
                          context.read<AdsBloc>().add(ShowInterstitialAdEvent());
                        } else if (state is InterstitialAdFailed) {
                          ScaffoldMessenger.of(context).showSnackBar(
                            SnackBar(content: Text('Failed to load ad: ${state.error}')),
                          );
                        }
                      });
                    },
                    child: const Text('Mostra Pubblicità'),
                  ),
                );
              }
            ),
  ...
  showDialog(
    context: context,
    builder: (BuildContext dialogContext) {  // Usa un nuovo contesto per il dialogo
      return AlertDialog(
             ...
             context.read<AdsBloc>().add(LoadRewardedAdEvent());
                      ...
                      context.read<AdsBloc>().stream.listen((state) {
                        if (state is RewardedAdLoaded) {
                          context.read<AdsBloc>().add(ShowRewardedAdEvent());
